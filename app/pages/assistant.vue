    ...
    <div class="assistant-page">
      ...
      <div class="chat-main">
        ...
        <div class="chat-messages" ref="messagesEl">
          ...
          <div v-if="messages.length === 0" class="welcome-state">
            ...
            <p class="welcome-sub">Describe your needs in plain English — budget, brand, how you'll use it — and I'll find the best matches on QatarLiving right now.</p>
      ...
      <aside class="rec-sidebar">
        ...
        <div v-else class="rec-list">
          <div
            v-for="(car, i) in recommendations"
            :key="car.id"
            class="rec-card"
            :class="{ 'rec-card-top': i === 0 }"
            @click="$router.push(`/cars/${car.id}`)"
          >
            <div class="rec-rank">#{{ i + 1 }}</div>
            <div class="rec-img">
              <svg viewBox="0 0 220 90" fill="none" class="rec-car-svg">
                <path d="M14 60 L14 48 Q20 32 38 24 L80 16 Q100 13 118 18 L158 24 Q178 28 192 44 L206 50 L206 60 Q206 70 198 70 L22 70 Q14 70 14 60Z" fill="rgba(200,169,110,0.07)" stroke="rgba(200,169,110,0.3)" stroke-width="1"/>
                <path d="M44 44 Q52 28 70 22 L108 16 L108 44Z" fill="rgba(150,200,240,0.05)" stroke="rgba(150,200,240,0.15)" stroke-width="0.8"/>
                <path d="M116 16 L148 20 L162 34 L166 44 L116 44Z" fill="rgba(150,200,240,0.05)" stroke="rgba(150,200,240,0.15)" stroke-width="0.8"/>
                <circle cx="62" cy="70" r="16" fill="#0e0e10" stroke="rgba(200,169,110,0.4)" stroke-width="1"/>
                <circle cx="62" cy="70" r="9" fill="rgba(200,169,110,0.05)" stroke="rgba(200,169,110,0.4)" stroke-width="0.8"/>
                <circle cx="158" cy="70" r="16" fill="#0e0e10" stroke="rgba(200,169,110,0.4)" stroke-width="1"/>
                <circle cx="158" cy="70" r="9" fill="rgba(200,169,110,0.05)" stroke="rgba(200,169,110,0.4)" stroke-width="0.8"/>
              </svg>
...
async function sendMessage(): Promise<void> {
  ...
  if (lower.includes('suv') || lower.includes('fortuner') || lower.includes('family')) {
    aiText = `I found <strong style="color:#c8a96e">23 matches</strong> for a family SUV in your range. Top picks are the Toyota Fortuner, Nissan Patrol, and Lexus RX 350.<br><br>The <strong>2021 Fortuner at QAR 98,000</strong> is ranked #1 — it's 7% below market average with only 42,000 km. Very strong deal right now.`
    suggestions = ['Show me the Fortuner details', 'Compare with Patrol', 'Filter under QAR 100k']
    newRecs = mockRecs.suv
  } else if (lower.includes('bmw') || lower.includes('mercedes') || lower.includes('luxury')) {
    aiText = `Found <strong style="color:#c8a96e">11 listings</strong> for BMW and Mercedes under QAR 150,000. The BMW X5 2020 at QAR 132,000 is the best value — well maintained, single owner.<br><br>Note: the Mercedes GLC listings have 2 suspicious ads I've filtered out automatically.`
    suggestions = ['Show BMW X5 details', 'Only Mercedes', 'Newer models only']
    newRecs = mockRecs.luxury
  } else if (lower.includes('price') || lower.includes('fair') || lower.includes('worth')) {
    aiText = `Based on <strong style="color:#c8a96e">12 comparable listings</strong> in Qatar right now, the fair market price for a 2021 Fortuner with ~40,000 km is around <strong>QAR 101,000–108,000</strong>.<br><br>Anything below QAR 100,000 is a good deal. Above QAR 115,000 would be overpriced for that year and mileage.`
    suggestions = ['Show me deals under QAR 100k', 'What about 2022 models?', 'Compare similar SUVs']
    newRecs = mockRecs.suv
  } else if (lower.includes('toyota') || lower.includes('low mileage')) {
    aiText = `I found <strong style="color:#c8a96e">18 Toyota listings</strong> with under 50,000 km. Best options right now are the Land Cruiser 2019, Fortuner 2021, and Camry 2022.<br><br>The Fortuner has the best mileage-to-price ratio in this batch.`
    suggestions = ['Show Land Cruiser', 'Fortuner under 40k km', 'Any Camry deals?']
    newRecs = mockRecs.toyota
  } else {
    aiText = `I searched QatarLiving for <strong style="color:#c8a96e">"${text}"</strong> and found some matching listings. Here are the top ranked results based on price, mileage, and condition.`
    suggestions = ['Show me the best deal', 'Filter by year', 'Show SUVs only']
    newRecs = mockRecs.suv
  }
...
.chat-title  { font-size: 15px; font-weight: 500; margin-bottom: 3px; }
.chat-status { display: flex; align-items: center; gap: 6px; font-size: 12px; color: var(--muted); }
...
.clear-btn:hover { color: var(--text) !important; }
...
.msg-suggestion:hover { background: rgba(200,169,110,0.08) !important; }

.msg-time { font-size: 11px; color: var(--muted2); padding: 0 4px; }
...
.input-row:focus-within { border-color: rgba(200,169,110,0.4); }
...
.chat-input :deep(textarea)::placeholder { color: var(--muted) !important; }
.chat-input :deep(div) { background: transparent !important; border: none !important; box-shadow: none !important; }
...
.send-btn:not(:disabled):hover { background: var(--accent2) !important; }
...
.sidebar-empty-icon { opacity: 0.3; color: var(--muted); }
.sidebar-empty-text { font-size: 13px; color: var(--muted); line-height: 1.6; max-width: 200px; }
...
.rec-card:hover { border-color: rgba(200,169,110,0.3); transform: translateY(-1px); }
.rec-card-top   { border-color: rgba(200,169,110,0.2); }
...
.rec-name  { font-size: 13px; font-weight: 500; margin-bottom: 4px; }
.rec-price {
  font-family: var(--font-display);
  font-size: 18px;
  color: var(--accent);
...
.rec-tags { display: flex; flex-wrap: wrap; gap: 5px; margin-bottom: 8px; }
.rec-tag {
  font-size: 10px;
  color: var(--muted);
  background: var(--bg4);
...
.rec-tag.good { color: var(--green); background: rgba(107,196,126,0.08); }
.rec-tag.warn { color: #e09a3a;      background: rgba(224,154,58,0.08); }
...
.rec-reason svg { flex-shrink: 0; color: var(--accent); opacity: 0.7; }
...
.view-all-btn:hover { border-color: rgba(200,169,110,0.3); color: var(--accent); }

/* ── Responsive ──────────────────────────────────────────── */
@media (max-width: 860px) {
  .assistant-page { grid-template-columns: 1fr; height: auto; overflow: visible; }
  .chat-main      { height: 70vh; }
  .rec-sidebar    { max-height: 60vh; }
}
</style>
